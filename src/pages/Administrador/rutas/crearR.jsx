// Handler genérico para los campos que no son empresa ni nombre
...
const CrearRuta = () => {
  ...
  // Estado y carga de rutas reales de la base de datos
  const [rutas, setRutas] = useState([])
  useEffect(() => {
    const token = localStorage.getItem('tokenAdmin')
    axios
      .get('/api/rutas/listar', { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleEmpresaChange = (e) => {
    ...
    setIdaList([]);
    setRetList([]);
    setErrors(errs => ({ ...errs, empresaId: validate('empresaId') }));
  };

  const handleNombreChange = (e) => {
    const nombre = e.target.value;
    const found = rutas.find(
      (r) => r.nombre === nombre && String(r.empresa?.id) === String(form.empresaId)
    );
    if (found) {
      setForm((prev) => ({
        ...prev,
      ...
      // Los paraderos pueden venir como objetos o ids
      const ida = Array.isArray(found.paraderos?.ida)
        ? found.paraderos.ida.map(p => typeof p === 'object' ? p.id : p)
        : [];
      const retorno = Array.isArray(found.paraderos?.retorno)
        ? found.paraderos.retorno.map(p => typeof p === 'object' ? p.id : p)
        : [];
      setIdaList(ida);
      setRetList(retorno);
    } else {
    ...
    setErrors(errs => ({ ...errs, nombre: validate('nombre') }));
  };
  ...
  const renderTags = (arr, fld) =>
    arr.map(id => {
      const p = paraderos.find(x => String(x.id) === String(id)) || {}
      return (
        ...
        <CForm onSubmit={handleSubmit} className="row g-3 form-ruta">
          ...
          <CCol md={6}>
            ...
            >
              ...
              {rutas
                .filter(r => String(r.empresa?.id) === String(form.empresaId))
                .map((ruta, idx) => (
